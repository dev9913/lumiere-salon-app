...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.booking.findUnique({ where: { id: params.id }, include: { service: true } });
  ...
  if (startsAt < new Date()) {
    return NextResponse.json({ error: "You can't reschedule to a time in the past." }, { status: 400 });
  }
  const endsAt = addMinutes(startsAt, existing.service.durationMins);
  ...
  try {
    ...
    console.error(err);
    return NextResponse.json({ error: "Something went wrong rescheduling your booking." }, { status: 500 });
  }
}
